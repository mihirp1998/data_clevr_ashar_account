...
# FOCAL_LENGTH_Y = 3.7320508075688776
# FOCAL_LENGTH_X = 3.7320508075688776

# FOCAL_LENGTH_Y =124.4
# FOCAL_LENGTH_X =70.0
# RADIUS = 12.0
# SCENE_SIZE =10.0
# NEAR = RADIUS - SCENE_SIZE
...
def transformer(voxels,
    ...
    def _interpolate(im, x, y, z, out_size):
        ...
        with tf.compat.v1.variable_scope('_interpolate'):
            ...
            x0_valid = tf.cast(
                tf.less_equal(x0, max_x) & tf.greater_equal(x0, 0), dtype=tf.float32)
            x1_valid = tf.cast(
                tf.less_equal(x1, max_x) & tf.greater_equal(x1, 0), dtype=tf.float32)
            y0_valid = tf.cast(
                tf.less_equal(y0, max_y) & tf.greater_equal(y0, 0), dtype=tf.float32)
            y1_valid = tf.cast(
                tf.less_equal(y1, max_y) & tf.greater_equal(y1, 0), dtype=tf.float32)
            z0_valid = tf.cast(
                tf.less_equal(z0, max_z) & tf.greater_equal(z0, 0), dtype=tf.float32)
            z1_valid = tf.cast(
                tf.less_equal(z1, max_z) & tf.greater_equal(z1, 0), dtype=tf.float32)
    ...
    def _meshgrid(depth, height, width, z_near, z_far):
        with tf.compat.v1.variable_scope('_meshgrid'):
            x_t = tf.reshape( #D x H x W
                tf.tile(tf.linspace(-SCENE_SIZE, SCENE_SIZE, width), [height * depth]),
                [depth, height, width])

            
            y_t = tf.reshape( #D x H x W
                tf.tile(tf.linspace(-SCENE_SIZE, SCENE_SIZE, height), [width * depth]),
                [depth, width, height])
            y_t = tf.transpose(a=y_t, perm=[0, 2, 1])
            ...
            z_t = tf.transpose(a=z_t, perm=[2, 0, 1])
    ...
    def _transform(theta, input_dim, out_size, z_near, z_far):
        with tf.compat.v1.variable_scope('_transform'):
            ...
            if do_project is not "invert":
                ...
                y_s = y_s * (1/SCENE_SIZE)
                x_s = x_s * (1/SCENE_SIZE)
            else:
                z_s = z_s/SCENE_SIZE
...
def get_transform_matrix_tf_(theta, phi, invert_rot = False, invert_focal = False):
    ...
    intrinsic_matrix = tf.linalg.tensor_diag(tf.constant(intrinsic_diag, dtype = tf.float32))
    
    camera_matrix = tf.matmul(extrinsic_matrix, intrinsic_matrix)
    return camera_matrix
    
    
def get_transform_matrix(theta, phi,FOCAL_LENGTH_Y,FOCAL_LENGTH_X ,invert_rot = False, invert_focal = False):
    ...
    if invert_focal:
        intrinsic_matrix[2, 2] = float(FOCAL_LENGTH_Y)
        intrinsic_matrix[1, 1] = float(FOCAL_LENGTH_X)
    else:
        intrinsic_matrix[2, 2] = 1.0 / float(FOCAL_LENGTH_Y)
        intrinsic_matrix[1, 1] = 1.0 / float(FOCAL_LENGTH_X)
...
def project_voxel(voxel,THETA,PHI,fov):
    ...
    rotmat = tf.constant(get_transform_matrix(THETA, PHI,FOCAL_LENGTH_Y,FOCAL_LENGTH_X), dtype = tf.float32)
...
def unproject_voxel(voxel):
    ...
    rotmat = tf.constant(get_transform_matrix(0.0, 0.0,FOCAL_LENGTH_Y,FOCAL_LENGTH_X, invert_focal = True), dtype = tf.float32)
    rotmat = tf.reshape(rotmat, (1, 4, 4))
    rotmat = tf.tile(rotmat, (BS, 1, 1))
...
def transformer_postprocess(voxel):
    ...
    voxel = tf.reverse(voxel, axis=[2, 3])
    #zxy -> xyz i think
    voxel = tf.transpose(a=voxel, perm=(0, 2, 3, 1, 4))
    return voxel

def voxel2mask_aligned(voxel):
    return tf.reduce_max(input_tensor=voxel, axis=3)
...
def voxel2depth_aligned(voxel):
    ...
    depth = tf.expand_dims(tf.argmin(input=costgrid, axis=3), axis=3)
...
def features_from_projected(voxel1, voxel2, factor):
    ...
    depth = tf.expand_dims(tf.argmin(input=costgrid, axis=3), axis = 3) #B x S x S x 1
    depth = tf.squeeze(tf.image.resize(depth, (Q, Q)), axis = 3) #B x Q x Q
    ...
    #BQQ x Q x 1
    depth = tf.nn.conv1d(input=depth, filters=filt, stride = 1, padding = 'SAME') 

    depth = tf.reshape(depth, (const.BS, Q, Q, Q, 1))
    
    depth_gated_feats = tf.reduce_sum(input_tensor=voxel2 * depth, axis = 3)

    return depth_gated_feats

def loss_against_constraint_l2(constraint, voxels):
    return tf.reduce_mean(input_tensor=constraint * voxels)    
...
def loss_against_constraint_ce(constraint, voxels):
    ...
    return tf.reduce_mean(input_tensor=-tf.math.log(voxels + const.eps) * constraint)
