...
from utils.tfutil import _bytes_feature, _int64_feature, _int64s_feature, _bytes_feature_list
...
IN_DIR = '../CLEVR_64_36_WITH_INSIDE/images/train/'
OUT_DIR = '../CLEVR_64_36_WITH_INSIDE/tfrs_withnames'
...
def convert_to_list(tree):
    ...
    words = [i['word'] for i in treex]
    function = [i['function'] for i in treex]
    children = [i['children'] for i in treex]
    bbox = [i['bbox'] for i in treex]
    return (words,function,children,bbox)
...
def np_for_obj(obj_path_):
    ...
    for phi in PHIS:
        for theta in THETAS:
            ...
            depth = np.zeros([const.H,const.W])
            depth = depth * (100 - 0) + 0
    ...
    tree_pickle = pickle.load(open(tree_path,"rb"))
    words,function,children,bbox = convert_to_list(tree_pickle)
    json_tree = {"words":words,"functions":function,"children":children,"bbox":bbox}
    ...
    return [np.stack(images, axis=0),
            ...
            binvox,folder_name,tree_path,json_tree_str]
...
def tf_for_obj(obj_np):
    ...
    # words,function,children,bbox = obj_np[7:]



    json_tree = obj_np[7].encode()
    ...
    example = tf.train.Example(features=tf.train.Features(feature={
        ...
        'obj_mask': _bytes_feature(mask.tostring()),

        "json_tree": _bytes_feature(json_tree),
        # 'voxel': _bytes_feature(voxel.tostring()),
        "voxel_obj" : _bytes_feature(voxel_obj.tostring()),
        'voxel_mask': _bytes_feature(np.array(val[str(num_objs)],np.float64).tostring()),
